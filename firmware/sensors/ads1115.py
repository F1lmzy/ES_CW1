...
class ADS1115:
    ...
    def __init__(self, bus: int = 1, address: int = ADS1115_ADDRESS, 
                 mock: bool = False):
        """
        ...
        if not self.mock:
            try:
                ...
                logger.info(f"ADS1115 initialized on I2C bus {bus}, address 0x{address:02X}")
        ...
        for attempt in range(retries):
            try:
                ...
                if attempt < retries - 1:
                    logger.warning(f"I2C write failed (attempt {attempt + 1}), retrying: {e}")
    ...
    def _read_register(self, pointer: int, retries: int = 3) -> int:
        ...
        for attempt in range(retries):
            try:
                ...
                if attempt < retries - 1:
                    logger.warning(f"I2C read failed (attempt {attempt + 1}), retrying: {e}")
