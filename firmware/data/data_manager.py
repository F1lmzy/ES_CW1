...
class DataManager:
    ...
    def store_reading(
        self, voltage: float, force_percent: float, state: str, variance: float
    ) -> bool:
        """
        ...
        try:
            ...
            if len(self._memory_queue) < self.MAX_QUEUE_SIZE:
                ...
                logger.warning(
                    f"Stored in memory queue (size: {len(self._memory_queue)})"
                )
            return False
    ...
    def get_recent_readings(
        self, limit: int = 100, hours: Optional[int] = None
    ) -> List[Dict]:
        """
    ...
    def cleanup_old_data(self) -> int:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if deleted > 0:
                    logger.info(
                        f"Cleaned up {deleted} old records (older than {self.RETENTION_DAYS} days)"
                    )
...
if __name__ == "__main__":
    ...
    # Save calibration
    dm.save_calibration(
        baseline_voltage=0.5, occupied_threshold=2.5, movement_threshold=0.1
    )
