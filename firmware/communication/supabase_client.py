...
class SupabaseClient:
    ...
    def _post(self, table: str, payload: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if response.status in (200, 201):
                ...
                logger.error(f"HTTP Error {response.status}: {response.read().decode()}")
                return False
    ...
    def insert_reading(self, reading_data: Dict[str, Any]) -> bool:
        ...
        payload = {k: v for k, v in reading_data.items() 
                  if k in ['created_at', 'device_id', 'user_id', 'voltage', 'force_percent', 'state', 'variance']}
